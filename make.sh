...
rm -f output.csv
osm-tag-csv-history -i "$FILENAME"  -o ./raw_csv_tag_changes.csv -t building -C old_value,new_value,iso_timestamp
...
psql -At -c "COPY (select * from t_to_pivot order by 1,2) TO STDOUT " | sort > tab
datamash -s crosstab 1,2 sum 3 < tab > crosstab_for_flouris.tsv
#
#rm -f tab
